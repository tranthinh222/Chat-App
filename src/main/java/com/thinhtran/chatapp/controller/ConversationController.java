...
public class ConversationController {
    ...
    public ResponseEntity<Page<ResMessageFullDto>> getAllMessages(@PathVariable Long id, Pageable pageable) {
        return ResponseEntity.status(HttpStatus.OK)
                .body(this.messageService.getMessages(id, SecurityUtil.getCurrentUserId().get(), pageable));
    }

    @PostMapping("/conversations")
    public ResponseEntity<ResConversationDto> createConversation(
            @Valid @RequestBody ReqCreateConversationDto reqCreateConversationDto) {
        return ResponseEntity.status(HttpStatus.CREATED)
                .body(this.conversationService.createConversation(reqCreateConversationDto));
    }

    @GetMapping("/sidebar")
    public ResponseEntity<List<ResSidebarDto>> getSideBar() {
        return ResponseEntity.status(HttpStatus.OK)
                .body(this.conversationService.getSideBar(SecurityUtil.getCurrentUserId().get()));
    }

    @PostMapping("/conversations/{id}/read")
    @ApiMessage("mark conversation as read")
    public ResponseEntity<?> markConversationAsRead(@PathVariable("id") Long conversationId) {
        Long userId = SecurityUtil.getCurrentUserId().get();
        this.messageService.markConversationAsRead(conversationId, userId);
        return ResponseEntity.status(HttpStatus.OK).body(Map.of("message", "Conversation marked as read"));
    }
}
